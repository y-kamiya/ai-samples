...
breakdown domain condition task@(Move a b) = case M.lookup task (compoundMap domain) of
...
execute domain condition (Stack x y pre post) = if satisfy pre condition then post else []
